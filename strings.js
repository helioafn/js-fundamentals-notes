// Strings
// Strings são textos, para não gerar nenhum erro
// precisam estar entre aspas simples ou duplas.
...
/*
    ...
    Um template literal parece uma string comum, mas invés de usar aspas simples
    ou duplas ('' ou "")
    se usa apóstrofos (``)
*/
let greeting = `Hello!`;

// `` Funciona normalmente como uma string, com exceção de que se pode
// incluir variaveis no template
...
// Template literais respeitam as quebras de linha no código fonte, logo
// se pode escrever strings
...
/*
    ...
    Valores primitivos como 'John Doe', não podem ter métodos ou propriedades
    (porque eles não são objetos).
    Mas em JavaScript, métodos e propriedades também estão disponíveis para
    valores primitivos, pois JavaScript trata valores primitivos como objetos
    quando está executando métodos e propriedades.
*/
...
// a diferença é que valores de inicio e fim menores que zero são tratados
// como 0 em substring()
...
// se quiser substituir todas as ocorrências, use expressões regulares (regEx)
// com a flag /g (global match)
...
// o método concat() pode ser usado invés do operador '+'. As linhas seguintes
...
// A especificação ECMAScript 2019 adicionou os métodos trimStart() e trimEnd()
// ao JavaScript esses métodos são similares a trim(), porém removem espaço
// somente em um dos lados da string
...
// A especificação ECMAScript 2017 adicionou dois métodos de string ao JavaScript:
// padStart() e padEnd()
...
// string.charCodeAt(indice) -> retorna o código UTF-16 (0 ~ 65535) do caractere
...
/*
    ...
    * Se nenhum caractere for encontrado, [] retorna undefined, enquanto
    * charAt() retorna uma string vazia.
